...
def findPeak(tr_times, filtered, path):
    ...
    if display == True:
        ...
        ax[2].plot(tr.times(), y, color = 'lightgrey')
        ...
        plt.savefig('plotsMars/' + path.removesuffix('.mseed') + '.png')  # Saves as PNG with high resolution
        plt.clf()

def readMINIseed(path):
    ...
    freqmax = frequencies[np.argmin(fft_magnitude)] * 1

    # print("max = ", freqmax, " min = ", freqmin)

    # print("frequencie with max magnitude = ", frequencies[np.argmax(fft_magnitude)] * 1.1)
    # print(frequencies[np.argmin(fft_magnitude)])
    # tr_filtered = signal.medfilt(tr_filtered, kernel_size = 201)

    # tr_filtered = np.abs(tr_filtered.filter('bandpass', freqmin = freqmin, freqmax = freqmax).copy())
    # tr_filtered = tr_filtered.filter('lowpass', freq = freqmin).copy()
    tr_filtered = tr_filtered.filter('highpass', freq = freqmin).copy()
    tr_filtered = np.abs(tr_filtered)

    return tr, tr_filtered
...
def objective_function(opt):
    ...
    b = [2 * 1e-9]      # Numerator (e.g., for a simple system)

    start = 0

    # Calculate the number of points based on the step size
    num_points = int((opt[2] - start) / tr.stats.sampling_rate) + 1

    # Generate the impulse response
    _ , imp_response = signal.impulse((b, a), T = np.linspace(start, opt[2], num_points))

    filtered = signal.convolve(tr.data, imp_response, mode='same')

    print("opt = ", opt, "fun = ", np.sum(np.abs(filtered)))

    return np.sum(np.abs(filtered))

def confFilter(directory_path, path):

    global tr
    
    print(path)

    tr, tr_filtered = readMINIseed(directory_path + path)
    ...
    findPeak(tr.times(), filtered, path)
...
if __name__ == "__main__":
    ...
    for file_name in file_names:
        confFilter(directory_path, file_name)
    print("")